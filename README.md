# Hey there! 👋

I'm [Maxime Capel](https://linkedin.com/in/maximecapel), a Computer Science Master's student at Université de Bordeaux, specializing in Computer Graphics and Audio (IIS - Informatique pour l'Image et le Son).

I focus on 3D graphics, image synthesis, and rendering techniques. I also have a strong foundation in full-stack development and game programming.

My main tech stack is C++, Python, and JavaScript. I work extensively with OpenGL for 3D rendering and image synthesis. I build full-stack web applications with modern frameworks and REST APIs, and develop games with Unity.

I've got several projects on my GitHub, with my favorites being [PokeVault](https://github.com/fortyup/pokevault), [Hexcore](https://github.com/fortyup/hexcore), and [Lumix](https://github.com/fortyup/lumix).

Currently working on advanced image synthesis projects and real-time rendering techniques. I'm exploring topics like virtual and augmented reality, geometry processing, and parallel programming for graphics applications.

My toolkit includes OpenGL and C++ for graphics programming, Python for image processing, JavaScript for web applications, Unity for game development, and Git for version control. I follow Agile workflows and enjoy working on innovative visual computing projects.

If you want to connect, reach out on [LinkedIn](https://linkedin.com/in/maximecapel) or shoot me an email at maxime.capel19@gmail.com.

---

💼 Open to internships and opportunities in computer graphics, image synthesis, VR/AR, and software development.
